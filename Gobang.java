...
class Gobang extends JFrame implements Runnable, ActionListener
{
 ...
 private boolean gobang_SpaceAI(int x, int y) /* 计算剩余空白位置 */
 {
  int space=0;
 ...
 private void setMark2(int x, int y) 
 {
  ...
  if(space+step>=5)
   ...
   setMark(x-(step+1)*displace_x, y-(step+1)*displace_y); /* 改下对点 */
 }
 ...
 public void paint(Graphics g)
 {
  ...
  switch(game_state)
  {
   case Start: g.drawString("2018.9.20  by qiankai", 91, 240);
               g.setFont(font);
 ...
 private void Load()
 {
  try
  {
   ...
   if(!file.exists())
   {
    JOptionPane.showMessageDialog(null, "沒有存档");
    return;
   }
